...
bot_token = "7884570852:AAHC5R1TqwWkli5pqXYsTKTu1uXjlCk15Iw"
    ...
    await app.reply_to(
        ...
        reply_markup=Keyboard([
            ...
            [
                ...
                Button("- SoundCloud -", callback_data="soundcloud") # @j4ck_721s & @j4ck_721
            ],
            [
                Button("- المطور -", url="j4ck_721.t.me")
            ]
        ])
    )
...
async def redefine(callback: CallbackQuery):
    ...
    await app.edit_message_reply_markup(
        ...
        message_id=callback.message.id,# @j4ck_721s & @j4ck_721
...
async def snap_vid(callback: CallbackQuery):
    ...
    await app.delete_message(user_id, answer.output.id)# @j4ck_721s & @j4ck_721
...
def tiktok(url):
    headers = {
        ...
        'sec-fetch-dest': 'empty',
        'sec-fetch-mode': 'cors',
        'sec-fetch-site': 'same-origin',# @j4ck_721s & @j4ck_721
...
def youtube(url):
    ...
    streams = yt.streams
    info = yt.vid_info
    return streams, info["videoDetails"] # @j4ck_721s & @j4ck_721
...
async def download(callback: CallbackQuery):
    ...
    thumbnail = info["thumbnail"]["thumbnails"][-1]["url"] # @j4ck_721s & @j4ck_721
    ...
    await app.send_video(
         ...
         caption=caption,# @j4ck_721s & @j4ck_721
    )
...
def soundcloud(url):
  ...
  return {
    "mp3" : mp3,
    "title" : title,
    "likes" : likes, # @j4ck_721s & @j4ck_721
    "username" : user,
  }
    ...
    for adminn in admins:
        try:
            await app.forward_message(
                ...
                message_id = message.id # @j4ck_721s & @j4ck_721
            )
        except:
            continue
    await app.reply_to(
